...
/*   Updated: 2021/06/12 22:09:49 by elvmarti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	color_int(t_cub *cub)
{
	...
	if (cub->color_f_r < 0 || cub->color_f_r > 255)
		print_error("Valor del color del suelo incorrecto");
...
static void	color_string(t_cub *cub, char *line, int *i)
{
	...
	if (line[*i] == ',')
		*i = *i + 1;
		...
		print_error("Falta una coma entre los valores del techo");
	if (!cub->cfr)
...
void	parse_floor(t_cub *cub, char *line)
{
	...
	while (line[i])
	{
		...
		if (line[i] != ' ' && line[i] && !(ft_isdigit(line[i])))
			print_error("Caracteres inválidos en los datos del suelo");
	}
	if (!cub->cfr || !cub->cfg || !cub->cfb)
		print_error("Faltan datos del suelo");
	color_int(cub);
}
